...
$PLACEHOLDER = '<Enter UPN Format>'
$mappingName = "VaultUsers_ALL.csv"
$mappingPath = Join-Path $ExportDir $mappingName

function Export-UsersForUPNMapping {
    param(
        [string[]]$UserTypes = @('EPVUser','EPVUserLite','BasicUser','ExtUser','BizUser')
    )

    $rows = New-Object System.Collections.Generic.List[object]

    foreach ($userType in $UserTypes) {
        Write-LogMessage -type Info -MSG "Retrieving Users under UserType: $userType"
        try {
            $resp = Invoke-RestMethod -Uri ("$($URL_Users)?UserType=$($userType)") -Method Get -Headers $s_pvwaLogonHeader -ErrorVariable pvwaERR
            Write-LogMessage -type Info -MSG "$userType $($resp.total)"

            if ($resp.total -gt 0) {
                # raw dump
                $resp.Users.username | Out-File (Join-Path $ExportDir "VaultUsers_$($userType).txt") -Force
    ...
    if ($rows.Count -eq 0) {
        Write-LogMessage -type Warning -MSG "No users returned. Nothing to export."
        return
    }

    $rows | Sort-Object Username | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $mappingPath -Force
    Write-LogMessage -type Success -MSG "Exported mapping CSV -> $mappingPath"
    Write-Host "`nOpen the CSV, replace '$PLACEHOLDER' in the NewUPN column, save it, then re-run and choose 'Update users from a file'." -ForegroundColor Green
}

function Get-LatestMappingCsvPath {
    if (Test-Path $mappingPath) { return $mappingPath }

    $pick = Read-Host 'Enter full path to a CSV to import (or press Enter to cancel)'
    if ([string]::IsNullOrWhiteSpace($pick)) { return $null }

    # Trim spaces
    $pick = $pick.Trim().Trim('"', "'")

    if (-not (Test-Path $pick)) {
        Write-LogMessage -type Error -MSG "File not found: $pick"
        return $null
    }
    return $pick
}
...
function Test-IsUPN([string]$Value) {
    # UPN check
...
function Update-UsersFromCsv {
    ...
    if (-not (Test-Path $Path)) {
        Write-LogMessage -type Error -MSG "CSV not found: $Path"
        return
    }

    $ts = Get-Date -Format 'yyyyMMdd_HHmmss'
    $backupPath = ($Path -replace '\.csv$','') + "_backup_$ts.csv"
    Copy-Item $Path $backupPath -Force | Out-Null
    Write-LogMessage -type Info -MSG "Backed up source CSV to $backupPath"
    ...
    foreach ($r in $rows) {
        ...
        if ([string]::IsNullOrWhiteSpace($newUPN) -or $newUPN -eq $PLACEHOLDER) {
            $skipped++
            $results.Add([pscustomobject]@{ Username=$username; UserId=$userId; AttemptedUPN=$newUPN; Status='Skipped - empty/placeholder' })
            continue
        }

        if (-not (Test-IsUPN $newUPN)) {
            $failed++
            $results.Add([pscustomobject]@{ Username=$username; UserId=$userId; AttemptedUPN=$newUPN; Status='Failed - invalid UPN format' })
            continue
        }

        # Get userId fallback on username
        if ([string]::IsNullOrWhiteSpace($userId)) {
            try {
                $lookup = Invoke-RestMethod -Uri ("$($URL_Users)?search=$([uri]::EscapeDataString($username))") -Method Get -Headers $s_pvwaLogonHeader
                $userId = ($lookup.Users | Where-Object Username -eq $username | Select-Object -First 1).Id
            } catch {}
        }

        if ([string]::IsNullOrWhiteSpace($userId)) {
            $failed++
            $results.Add([pscustomobject]@{ Username=$username; UserId=''; AttemptedUPN=$newUPN; Status='Failed - UserId not found' })
            continue
        }

        $body = @{ username = $newUPN } | ConvertTo-Json -Compress

        try {
            $null = Invoke-RestMethod -Method Put -Uri ("$($URL_Users)/$userId") -Headers $s_pvwaLogonHeader -ContentType "application/json" -Body $body
            $updated++
            $results.Add([pscustomobject]@{ Username=$username; UserId=$userId; AttemptedUPN=$newUPN; Status='Updated' })
        }
        catch {
            $failed++
            $results.Add([pscustomobject]@{ Username=$username; UserId=$userId; AttemptedUPN=$newUPN; Status=("Failed - " + (Join-ExceptionMessage $_.Exception)) })
        }
    }

    $summaryPath = Join-Path $ExportDir ("UpdateResults_{0}.csv" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
    $results | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $summaryPath
    Write-LogMessage -type Success -MSG "Done. Updated: $updated; Skipped: $skipped; Failed: $failed. Results -> $summaryPath"
}

# ---------------- Menu ----------------
$choice = Get-Choice -Title "SAM To UPN Utility (v$scriptVersion)" -Options @(
    "Get list of users from Vault",
    "Update users from a file",
    "Exit"
) -DefaultChoice 1

switch ($choice) {
    "Get list of users from Vault" { Export-UsersForUPNMapping; break }
    "Update users from a file" {
        $path = Get-LatestMappingCsvPath
        if ($null -ne $path) { Update-UsersFromCsv -Path $path }
        break
    }
    default { Write-LogMessage -type Info -MSG "Bye."; break }
}
