...
Function Stop-CYBRService
{
    ...
    try{
        ...
        #get process ID
        $Service = (Get-WmiObject win32_service | Where-Object {$_.name -eq $ServiceName} -ErrorAction SilentlyContinue).ProcessID
        if($Service -eq 0){
...
Function Find-UserInSystemLogs
{
    ...
    ForEach($log in $LogPaths)
    {
        ...
        {
            If(Get-Content -Path $log | Where-Object {$_ -Match $User}){
                $retUser = $Matches[0]
                break
            }
        }
    ...
    try{
        ...
        Foreach($credFile in $Component.ComponentUser)
        {
            ...
            if([string]::IsNullOrEmpty($ComponentUser))
            {
                ...
                If($offlineComponents.Count -eq 0 -or $null -eq $ComponentUser)
                {
